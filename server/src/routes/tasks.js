...
// Returns task instances; if none exist, initialises them from templates.
router.get('/airplane/:airplaneId/station/:stationId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Auto-initialise from active templates if none exist
    if (!instances || instances.length === 0) {
      const templates = await query(
        'SELECT * FROM task_templates WHERE station_id = ? AND active = TRUE ORDER BY order_index',
        [stationId]
      );
      for (const t of templates) {
        await query(
          'INSERT INTO task_instances (airplane_id, template_id, station_id) VALUES (?,?,?)',
          [airplaneId, t.id, stationId]
        );
      }
    ...
    // Enrich with time totals, signoffs, and active NCRs
    const enriched = await Promise.all(instances.map(async (inst) => {
      ...
      const hasHighNcr = ncrs.some(n => n.severity === 'high');
      return {
        ...inst,
...
// PUT /api/tasks/:id — update status or notes
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Enforce sequential ordering — task cannot move past not_started until all prior tasks done
    if (status && status !== 'not_started') {
      const priorIncomplete = await query(
        `SELECT COUNT(*) AS cnt FROM task_instances ti
         JOIN task_templates tt ON ti.template_id = tt.id
         WHERE ti.airplane_id = ? AND ti.station_id = ? AND tt.order_index < (
           SELECT order_index FROM task_templates WHERE id = ?
    ...
    const updated = await query(
      `SELECT ti.*, tt.title, tt.description, tt.estimated_minutes, tt.order_index
       FROM task_instances ti JOIN task_templates tt ON ti.template_id = tt.id
       WHERE ti.id = ?`,
      [req.params.id]
    );
    res.json(updated[0]);
...
router.post('/:id/signoff', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Return updated task with signoffs
    const updated = await query(
      `SELECT ti.*, tt.title, tt.description, tt.estimated_minutes, tt.order_index
       FROM task_instances ti JOIN task_templates tt ON ti.template_id = tt.id
       WHERE ti.id = ?`,
    ...
    res.json({ task: updated[0], signoffs: updatedSignoffs });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
  }
